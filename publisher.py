"""
Instagram Publisher - FIXED v3
Fixes token parse error by stripping whitespace
"""
import os
import time
import requests
...
def upload_fileio(image_path):
    try:
        with open(image_path, 'rb') as f:
            r = requests.post("https://file.io", files={"file": f}, timeout=40)
        j = r.json()
        if j.get("success"):
            return j.get("link")
    except Exception as e:
...
def get_public_url(image_path):
    for func in [upload_tmpfiles, upload_fileio]:
        url = func(image_path)
